...
export default function NursePage() {
  ...
  const [dbSurgeons, setDbSurgeons] = useState([]);
  ...
  const [form, setForm] = useState({
    ...
    patientId: '', surgeonId: '', procedureType: '', bodyLocation: '',
  ...
  const generateImplantId = useCallback(async (hospitalId) => {
    try {
      const hospId = hospitalId || user?.hospitalId || 'HOSP';
      const code = hospId.split(' ')[0].substring(0,3).toUpperCase();
      const all = await api.getImplantsByHospital(hospId);
  ...
  const generateEventId = () => {
    const hospCode = (user?.hospitalId||'HOSP').split(' ')[0].substring(0,3).toUpperCase();
  ...
  const loadTodayCases = useCallback(async () => {
    ...
    try {
      const data = await api.getCases({ date: today, hospitalId: user?.hospitalId });
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [c, s, l] = await Promise.allSettled([
        api.getConsignments({}),
        api.getSurgeons(user?.hospitalId),
        api.getLookupValues(),
      ]);
      if (c.status==='fulfilled') setConsignments((c.value||[]).filter(c=>c.status==='active'));
      if (s.status==='fulfilled') setDbSurgeons(s.value||[]);
  ...
  useEffect(() => {
    if (!user?.role) return;
    refresh();
    generateImplantId(user?.hospitalId).then(id=>set('implantId',id));
    setAdverseForm(f=>({...f, eventId: generateEventId()}));
    loadTodayCases();
  }, [refresh, user?.hospitalId, generateImplantId, loadTodayCases]);
  ...
  const handleCaseSelect = (caseId) => {
    ...
    // Set ALL fields at once — do NOT call handleConsignmentSelect (it resets surgeon/procedure)
    // Instead set form fields directly, preserving case-populated values
    setForm(f => ({
      ...f,
      caseId,
      surgeonId:       cs.surgeon_id     || f.surgeonId,
    ...
    // Set surgeon detail card
    if (cs.surgeon_id) {
      const found = dbSurgeons.find(s => s.surgeon_id === cs.surgeon_id);
      setSelectedSurgeon(found || null);
    }
  ...
  const handleConsignmentSelect = (consignmentId) => {
    ...
    set('procedureType', ''); set('bodyLocation', ''); set('surgeonId', '');
  ...
  const submitImplant = async (e) => {
    ...
    try {
      ...
      const nextId = await generateImplantId(user?.hospitalId);
      setSelectedSurgeon(null); setDeviceCategory(''); setDeviceType('');
      setForm({ implantId:nextId, consignmentId:'', udiPI:'', lotNumber:'', serialNumber:'',
        patientId:'', surgeonId:'', procedureType:'', bodyLocation:'', procedureDate:today, notes:'', caseId:'' });
      refresh();
  ...
  const draftMDR = async (event) => {
    ...
    try {
      ...
      try {
        ...
        if (history?.length>0) {
          ...
          enriched = { ...enriched, serialNumber:implant.serialNumber||'', procedureDate:implant.procedureDate||'',
            surgeonId:implant.surgeonId||'', bodyLocation:implant.bodyLocation||'',
      ...
      const resp = await fetch('/api/ai/complete', {
        ...
        body: JSON.stringify({ prompt:`Write ONLY the clinical narrative for Section B.5 of FDA Form 3500A. 150-250 words, factual, third person past tense. Device: ${enriched.deviceName}, Event: ${enriched.eventType}, Date: ${enriched.eventDate}, Hospital: ${enriched.hospitalId}. Description: ${enriched.description||'N/A'}`, maxTokens:400 }),
      });
    ...
    try {
      const resp = await fetch('/api/ai/mdr-pdf', {
        ...
        body: JSON.stringify({ narrative, eventDate:event.eventDate, today:new Date().toISOString().split('T')[0],
          deviceName:event.deviceName, lotNumber:event.lotNumber, hospitalId:event.hospitalId,
  ...
  return (
    <>
      ...
      <div style={{display:'flex',gap:4,marginBottom:20,background:'var(--bg-card)',
        ...
        <button style={tabStyle('implant')}   onClick={()=>switchTab('implant')}>💊 Record Implant</button>
      ...
      {tab==='implant' && (
        <div className="card">
          <div className="card-header">
            <span className="card-title">💊 Record Implant</span>
          ...
          <form onSubmit={submitImplant}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(10,1fr)',gap:16,marginBottom:14}}>
              ...
              <div className="form-group" style={{gridColumn:'span 10'}}>
                <label style={{fontSize:13,fontWeight:700,color:'var(--accent-amber)'}}>
                  ...
                  <span style={{fontSize:11,fontWeight:400,color:'var(--text-muted)',marginLeft:8}}>
                    optional — auto-populates surgeon, procedure, date and patient MRN
                  </span>
                </label>
                <select value={form.caseId} onChange={e=>handleCaseSelect(e.target.value)}
                  ...
                  {todayCases.filter(cs=>cs.status!=='completed'&&cs.status!=='cancelled').map(cs=>(
                    <option key={cs.case_id} value={cs.case_id}>
                      {cs.case_id} · {cs.procedure_type||'—'} · {cs.surgeon_id||'—'}{cs.or_room?' · OR '+cs.or_room:''}
                    </option>
                  ))}
                </select>
                {form.caseId && (() => {
              ...
              <div className="form-group" style={{gridColumn:'span 8'}}>
                <label>Consignment (Device at this Hospital)</label>
              ...
              <div className="form-group" style={{gridColumn:'span 3'}}>
                <label>Surgeon</label>
                <input list="surgeon-list" placeholder="Type surgeon ID..." value={form.surgeonId}
                  onChange={e=>{ set('surgeonId',e.target.value); const found=dbSurgeons.find(s=>s.surgeon_id===e.target.value); setSelectedSurgeon(found||null); }}/>
                <datalist id="surgeon-list">
                  {dbSurgeons.filter(s=>{
                    ...
                    return true;
                  }).map(s=><option key={s.surgeon_id} value={s.surgeon_id}/>)}
                </datalist>
                {selectedSurgeon && (
                  <div style={{marginTop:6,padding:'8px 12px',background:'var(--bg-secondary)',
                    ...
                    fontSize:12,color:'var(--text-secondary)',display:'flex',gap:16,flexWrap:'wrap'}}>
                    <span>👤 <strong style={{color:'var(--text-primary)'}}>{selectedSurgeon.full_name}</strong></span>
                    <span>🏥 {(selectedSurgeon.hospitals||[]).join(', ')||'—'}</span>
                    <span>🔬 {selectedSurgeon.specialty||'—'}</span>
                    {selectedSurgeon.npi && <span>NPI: {selectedSurgeon.npi}</span>}
                  </div>
                )}
              </div>
              <div className="form-group" style={{gridColumn:'span 4'}}>
              ...
              <div className="form-group" style={{gridColumn:'span 5'}}>
                ...
                <input list="location-list" value={form.bodyLocation} onChange={e=>set('bodyLocation',e.target.value)}
                ...
                <datalist id="location-list">
                  {(()=>{
                    ...
                    const deviceLocs=cons?.bodyLocations?.length>0?cons.bodyLocations:null;
            ...
            <button type="submit" className="btn btn-primary" style={{background:'var(--accent-amber)'}}
              disabled={busy||!form.consignmentId||!form.patientId||!form.procedureType||!form.bodyLocation}>
              {busy?<><span className="spinner" style={{width:14,height:14}}/> Recording…</>:'✓ Record Implant on Blockchain'}
            </button>
          </form>
        </div>
      )}
        ...
        <div className="card">
          ...
          <form onSubmit={submitExplant}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(10,1fr)',gap:16,marginBottom:14}}>
              ...
              <div className="form-group" style={{gridColumn:'span 3'}}>
                <label>Explanting Surgeon</label>
                <input list="explant-surgeon-list" placeholder="Type surgeon ID..." value={explantForm.explantedBy} onChange={e=>setE('explantedBy',e.target.value)}/>
                <datalist id="explant-surgeon-list">{dbSurgeons.map(s=><option key={s.surgeon_id} value={s.surgeon_id}/>)}</datalist>
              </div>
              <div className="form-group" style={{gridColumn:'span 3'}}>
        ...
        <>
          ...
          <div className="card" style={{marginTop:16}}>
            ...
            {adverseEvents.length===0
              ? <div className="empty-state" style={{padding:'16px 0'}}><div className="icon">📋</div><p>No adverse events recorded for your hospital yet</p></div>
              : <div className="table-wrap"><table>
                  <thead><tr><th>Event ID</th><th>Implant ID</th><th>Device</th><th>Type</th><th>Date</th><th>Hospital</th><th>Reported to FDA</th><th>Status</th><th>Actions</th></tr></thead>
      ...
      {tab==='patient' && (
        <div className="card">
          ...
          {implants.filter(i=>(!dateFrom||i.procedureDate>=dateFrom)&&(!dateTo||i.procedureDate<=dateTo)).length>0 && (
            <div className="table-wrap"><table>
              <thead><tr><th>Implant ID</th><th>Device</th><th>Category</th><th>Lot #</th><th>Serial #</th><th>Procedure</th><th>Body Location</th><th>Date</th><th>Hospital</th><th>Status</th><th></th></tr></thead>
                        ...
                        <div style={{padding:'16px 20px',borderLeft:'3px solid var(--accent-amber)'}}>
                          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'10px 24px'}}>
                            {[['Implant ID',i.implantId,true],['UDI-DI',i.udiDI,true],['Lot #',i.lotNumber,true],['Serial #',i.serialNumber||'—',true],
                              ['Procedure',i.procedureType,false],['Body Location',i.bodyLocation,false],['Surgeon',i.surgeonId||'—',false],['Hospital',i.hospitalId,false]].map(([label,val,mono])=>(
                              <div key={label}>
        ...
        <div className="card">
          ...
          {loading
            ...
            : consignments.length===0
              ...
              : <div className="table-wrap"><table>
                  <thead><tr><th>Consignment ID</th><th>Device</th><th>Category</th><th>Hospital</th><th>Location</th><th>Lot #</th><th>Total</th><th>Used</th><th>Available</th><th>Sterile Expiry</th></tr></thead>
                  <tbody>{[...consignments].sort((a,b)=>a.deviceCategory.localeCompare(b.deviceCategory)||a.deviceName.localeCompare(b.deviceName)).map(c=>{
                    ...
                    return (
                      <tr key={c.consignmentId} style={{cursor:'pointer',background:cat.bg}}
                        ...
                        <td style={{fontSize:11}}>🏥 {c.hospitalId}</td>
          ...
          <p style={{fontSize:11,color:'var(--text-muted)',marginTop:8}}>Click any row to pre-select that consignment in Record Implant</p>
        </div>
      )}
      ...
      {tab==='cases' && (
        <div className="card">
          ...
          <div className="alert alert-info" style={{marginBottom:12,fontSize:12}}>
            ℹ Click "Record Implant" on a case to pre-select it — the implant will automatically link to the case when recorded.
          </div>
          {casesLoading
            ? <div className="loading-overlay"><span className="spinner"/></div>
            : todayCases.length===0

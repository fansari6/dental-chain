...
export default function InfectionPrevPage() {
  ...
  const isSystemLevel = !user?.hospitalId;
    ...
    try {
      ...
      setLotMsg({
        ...
        text: (results||[]).length > 0
          ...
          : `✅ No active implants found for lot "${lot.lotNumber}"${user?.hospitalId ? ` at ${user.hospitalId}` : ''}`,
      });
  ...
  const notifyAll = async () => {
    ...
    try {
      const result = await api.bulkRecallNotification({
        ...
        notes: notifNotes,
        implants: unnotified.map(r => ({ implantId: r.implantId, patientIdHash: r.patientIdHash||'', hospitalId: r.hospitalId })),
      });
  ...
  const notifySingle = async (implant) => {
    ...
    try {
      ...
      await api.recordRecallNotification({
        ...
        patientIdHash: implant.patientIdHash||'', hospitalId: implant.hospitalId,
  ...
  const analyzeRecallImpact = async () => {
    ...
    try {
      const summary = lotResults.map((r,i) => ({
        ...
        hospital:r.hospitalId, status:r.status||'implanted',
      }));
  ...
  const draftMDR = async (event) => {
    ...
    try {
      const resp = await fetch('/api/ai/complete', {
        method:'POST', credentials:'include',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ prompt:`Write ONLY the clinical narrative for Section B.5 of FDA Form 3500A. 150-250 words, factual, third person past tense. Device: ${event.deviceName}, Event: ${event.eventType}, Date: ${event.eventDate}, Hospital: ${event.hospitalId}. Description: ${event.description||'N/A'}`, maxTokens:400 }),
      });
      const data = await resp.json();
  ...
  return (
    <>
      <div className="page-header">
        <h2>🔬 Infection Prevention Portal</h2>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <span className={`badge ${isSystemLevel?'badge-blue':'badge-purple'}`} style={{fontSize:11}}>
            {isSystemLevel ? '🌐 System Level' : `🏥 ${user?.hospitalId}`}
          </span>
        </div>
      </div>
              ...
              <>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                  <div style={{fontSize:13,fontWeight:600,color:'var(--accent-red)'}}>
                    {lotResults.length} patient{lotResults.length>1?'s':''} — {user?.hospitalId||'all hospitals'}
                  </div>
                  <div style={{display:'flex',gap:8}}>
                ...
                <div className="table-wrap"><table>
                  <thead><tr>
                    ...
                    <th>Body Location</th><th>Date</th><th>Hospital</th><th>Action</th>
                  </tr></thead>
            ...
            <>
              ...
              <div className="table-wrap"><table>
                <thead><tr>
                  ...
                  <th>Notified By</th><th>Hospital</th><th>Notes</th><th>Timestamp</th>
                </tr></thead>
                <tbody>{notifications.map((n,i)=>(
                  <tr key={i}>
                    ...
                    <td style={{fontSize:11}}>🏥 {n.hospitalId}</td>
      ...
      {tab==='mdr' && (
        <div className="card">
          ...
          <div className="alert alert-info" style={{marginBottom:12,fontSize:12}}>
            ...
            {user?.hospitalId && <span style={{marginLeft:8,color:'var(--accent-purple)'}}>Showing: 🏥 {user.hospitalId}</span>}
          </div>
              ...
              : mdrDeadlines.length === 0
                ...
                : <div className="table-wrap"><table>
                    <thead><tr>
                      ...
                      <th>Days Remaining</th><th>Hospital</th><th>Actions</th>
                    </tr></thead>
        ...
        <div className="card">
          ...
          {patientResults.length > 0 && (
            <>
              ...
              <div className="table-wrap"><table>
                <thead><tr><th>Implant ID</th><th>Device</th><th>Category</th><th>Lot #</th><th>Procedure</th><th>Body Location</th><th>Date</th><th>Hospital</th><th>Status</th></tr></thead>
      ...
      {tab==='adverse' && (
        <div className="card">
          ...
          <p style={{fontSize:12,color:'var(--text-secondary)',marginBottom:16}}>
            Device-related adverse events for {user?.hospitalId||'all hospitals'}. Class I/II must be reported within 30 days (21 CFR Part 803).
          </p>
          {adverseLoading
            ? <div className="loading-overlay"><span className="spinner"/></div>
            : !adverseLoaded

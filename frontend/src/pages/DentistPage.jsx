            ...
            <div style={{padding:'16px 20px',borderLeft:'3px solid var(--accent-blue)'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'10px 24px',marginBottom:12}}>
                {[
                  ...
                  ['Implanting Dentist', i.dentistId || '—',    false],
                  ['Practice',         i.practiceId,            false],
                ...
                <div style={{padding:'12px 14px',background:'rgba(245,158,11,0.06)',borderRadius:'var(--radius-sm)',border:'1px solid rgba(245,158,11,0.25)'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'10px 24px'}}>
                    {[
                      ...
                      ['Explanting Dentist',i.explantedBy    || '—', false],
...
export default function DentistPage() {
  ...
  const loadMyImplants = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.getImplantsByDentist(user?.username);
      ...
      try {
        ...
        if (history?.length > 0) {
          ...
          enriched = { ...enriched, serialNumber: implant.serialNumber||'',
            procedureDate: implant.procedureDate||'', dentistId: implant.dentistId||'',
...
Write only the narrative. No headers, no bullets. Third person past tense.`;
    ...
    try {
      const resp = await fetch('/api/ai/mdr-pdf', {
        ...
        body: JSON.stringify({
          ...
          practiceId: event.practiceId, eventId: event.eventId,
  ...
  const filteredImplants = myImplants.filter(i => {
    ...
    if (practiceFilter && i.practiceId !== practiceFilter) return false;
  ...
  const practices  = [...new Set(myImplants.map(i=>i.practiceId))].sort();
  ...
  const byPractice = practices.reduce((acc, h) => {
    acc[h] = myImplants.filter(i=>i.practiceId===h).length;
    return acc;
  }, {});
  ...
  const implantTable = (implants, colSpan=10) => (
    <div className="table-wrap"><table>
      <thead><tr>
        ...
        <th>Body Location</th><th>Lot #</th><th>Date</th><th>Practice</th>
        <th>Status</th><th></th>
      </tr></thead>
      <tbody>{(() => {
        let lastCat = null;
                ...
                onDraftMDR={async (aeId) => {
                  ...
                  if (ae) draftMDR(ae);
                    ...
                    draftMDR({ eventId: aeId, implantId: i.implantId,
                      ...
                      practiceId: i.practiceId, eventType: 'malfunction',
    ...
    <>
      ...
      {tab === 'patients' && (
        <div className="card">
          ...
          <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
            ...
            <select value={practiceFilter} onChange={e=>setPracticeFilter(e.target.value)} style={{minWidth:180}}>
              <option value="">All Practices</option>
              {practices.map(h=><option key={h} value={h}>{h}</option>)}
            </select>
            ...
            {(search||categoryFilter||practiceFilter||statusFilter) && (
              <button className="btn btn-ghost btn-sm"
                onClick={()=>{setSearch('');setCategoryFilter('');setPracticeFilter('');setStatusFilter('');}}>
                ✕ Clear
              </button>
            )}
          ...
          {loading
            ...
            : myImplants.length === 0
              ? <div className="empty-state"><div className="icon">🩺</div>
                  <p>No implant records found for your dentist ID</p>
                  <p style={{fontSize:12}}>Records appear here when your dentist ID is set on implant records</p>
                </div>
      ...
      {tab === 'lookup' && (
        <div className="card">
          ...
          <div className="alert alert-info" style={{marginBottom:16,fontSize:12}}>
            ℹ Shows all implants for the patient regardless of dentist — essential for pre-operative assessment.
            ...
            : adverseEvents.length === 0
              ...
              : <div className="table-wrap"><table>
                  <thead><tr>
                    ...
                    <th>Date</th><th>Practice</th><th>Reported to FDA</th><th>Status</th><th>Actions</th>
                  </tr></thead>
      ...
      {tab === 'analytics' && (
        <>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16}}>
            ...
            {/* By Practice */}
            <div className="card">
              <div className="card-header"><span className="card-title">Procedures by Practice</span></div>
              {practices.length === 0
                ? <div className="empty-state"><div className="icon">🏥</div><p>No data</p></div>
                : <div className="table-wrap"><table>
                    <thead><tr><th>Practice</th><th>Procedures</th><th>%</th></tr></thead>
                    <tbody>{practices.map(h=>{
                      const cnt = byPractice[h] || 0;
                      const pct = totalProcedures > 0 ? Math.round((cnt/totalProcedures)*100) : 0;
                      return (
                        <tr key={h}>
